...
class ManageFramer extends Component {
  constructor(props){
    super(props)
    this.state={
      framerUser:'',
  ...
  render(){
    const userManageFramer = [
      ...
      { title: "认证日期", dataIndex: "checker_time" , key: "checker_time" , width:"14%",
          render:(text,record) =>(<b>{record.checker_time}</b>)
      },
    ...
    return(
      <div className="user_manage_framer_box">
        <SelectAndTable columns={userManageFramer}
          title={
            <h3>
              <span>创作者：<b>{this.state.framerUser}</b></span>
            ...
            uri='/api/admins/user/manage/creater'
  ...
  componentDidMount(){
    Fetch.post({
      uri:'/api/admins/user/manage/creater',
      callback:(res)=>{
        // console.log(res,'chuangzuozhe');
        this.setState({
          framerUser:res.createrNum,
          girl:res.womenCreaterProportion,
          boy:res.manCreaterProportion,
          todayAdd:res.todayNewCreaterNum,
          todyGirl:res.todayNewWomenCreaterNum,
          todyBoy:res.todayNewManCreaterNum,
