...
class ManageCensor extends Component {
  constructor(props){
    super(props)
    this.state={
      censorUser:'',
  ...
  render(){
    ...
    return(
      <div className="user_manage_censor_box">
        <SelectAndTable columns={userManageCensor}
          title={
            <h3>
              <span>审查者：<b>{this.state.censorUser}</b></span>
            ...
            uri='/api/admins/user/manage/investigate'
  ...
  componentDidMount(){
    Fetch.post({
      ...
      callback:(res)=>{
        console.log(res,'审查');
        this.setState({
          censorUser:res.investigateNum,
          girl:res.womenInvestigateProportion,
          boy:res.manInvestigateProportion,
          todayAdd:res.todayNewInvestigateNum,
          todyGirl:res.todayNewWomenInvestigateNum,
          todyBoy:res.todayNewManInvestigateNum,
          BatchName:res.batchBehaivor
        })
      }
    })
  }
