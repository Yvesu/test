...
class ManageMechanism extends Component {
  constructor(props){
    super(props)
    this.state={
      mechanism:'',
  ...
  render(){
    ...
    return(
      <div className="user_manage_mechanism_box">
        <SelectAndTable columns={userManageMechanism}
          title={
            <h3>
              <span>机构：<b>{this.state.mechanism}</b></span>
            ...
            uri='/api/admins/user/manage/organization'
  ...
  componentDidMount(){
    Fetch.post({
      ...
      callback:(res)=>{
        console.log(res,'机构');
        this.setState({
          mechanism:res.organizationUserNum,
          girl:res.organizationWomenProportion,
          boy:res.organizationManProportion,
          todayAdd:res.todayNewOrganizationNum,
          todyGirl:res.todayWomen,
          todyBoy:res.todayNewOrganizationWomenNum,
...
export default ManageMechanism
