    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                ...
                <li>
                    <a class="link-effect" href="{{ asset('/admin/log/maintain') }}">
                        {{ trans('content.maintain-info').trans('common.manage_log') }}
                    </a>
                </li>
            </ol>
        </div>
    ...
    <div class="block">
        <div class="block-content tab-content">
            <div class="col-md-12 col-xs-12 block">
                {{--搜索--}}
                <form action="/admin/log/maintain" method="get">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tr>
                        ...
                        <th class="hidden-xs col-md-1 text-center">{{ trans('common.title') }}</th>
                        <th class="col-md-2">{{ trans('common.icon') }}</th>
                        <th class="hidden-xs col-md-1 text-center">{{ trans('common.keywords') }}</th>
                    ...
                    @foreach($datas as $k=>$data)
                        <tr>
                            ...
                            <td class="hidden-xs text-center">{{ $data->title ? $data->title : trans('content.none') }}</td>
                            <td class="hidden-xs text-center" style="height:20px;">
                                @if($data->icon)
                                    <img class="img-responsive" src="{{ CloudStorage::downloadUrl($data->icon) }}" style="width:40px;">
                                @else
                                    {{ trans('content.none') }}
                                @endif
                            </td>
                            <td class="hidden-xs text-center">{{ $data->keywords ? $data->keywords : trans('content.none') }}</td>
