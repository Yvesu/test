    ...
    <div class="row items-push">
        ...
        <div class="col-sm-4 col-xs-9">
            <h1 class="page-heading" >
                {{trans('common.video')}} : {{ $video->name }}
            </h1>
        </div>
        <div class="col-sm-8 col-xs-3">
            <!-- Single button -->
            <div class="btn-group pull-right" id="family-action">
                <a type="button" class="btn bg-red" href="{{ asset('admin/content/video/' . $video->id) }}">
                    {{ trans('common.cancel') }}
                </a>
            </div>
        </div>
    ...
    <div class="row" style="margin-top: 5%;">
        <div class="col-md-10 col-xs-12 block col-md-offset-1">
            <form class="js-validation-channel form-horizontal"
                  ...
                  action="{{ asset('/admin/content/video/'. $video->id) }}"
                  method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token()}}">
                <input type="hidden" name="_time" value="{{ strtotime($video->updated_at) }}">
