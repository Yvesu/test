    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                ...
                <li><a class="link-effect" href="">{{trans('content.edit_activity')}}</a></li>
            </ol>
        </div>
        <div class="col-sm-4 col-xs-9">
            <h1 class="page-heading" >
                {{trans('content.edit_activity')}} : {{ $topic->name }}
            </h1>
        </div>
    ...
    <div class="row" style="margin-top: 5%;">
        <div class="col-md-10 col-xs-12 block col-md-offset-1">
            <form class="js-validation-topic form-horizontal"
                  ...
                  action="{{ asset('/admin/content/activity/'. $topic->id) }}"
                  method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token()}}">
                <div class="form-group" style="margin-top:5%">
                    <div class="col-md-8 col-md-offset-2">
                        <label for="name">{{ trans('content.activity_name') }} <span class="text-danger">*</span></label>
                        <div class="form-material form-material-primary">
                            <input type="text" class="form-control" id="name" name="name" value="{{ $topic->name }}" disabled>
                        </div>
                        @if(Session::has('name'))
                ...
                <div class="form-group">
                    <div class="col-md-8 col-md-offset-2">
                        <div class="form-material">
                            <textarea class="form-control" id="comment" name="comment" rows="2">{{ $topic->comment }}</textarea>
                            <label for="comment">{{ trans('content.activity_comment') }}</label>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <div class="col-md-4 col-md-offset-4 col-xs-12" style="margin-top: 20px">
                        <div class="img-container fx-opt-zoom-in">
                            <img class="img-responsive img-thumbnail center-block" id="icon" src="{{ CloudStorage::downloadUrl($topic->icon) }}" alt="" style="width:150px;">
                                ...
                                <div class="img-options-content">
                                    <a href="#" class="btn button-change-profile-picture">
                                        <label for="upload-profile-picture">
                                            ...
                                            <input class="btn-upload" name="topic-icon" id="topic-icon" type="file" title style="margin-top:-32px;" onchange="PreviewImage.topic_preview(this,'icon','{{CloudStorage::downloadUrl($topic->icon)}}')">
                                        </label>
                                    </a>
                                </div>
                            </div>
