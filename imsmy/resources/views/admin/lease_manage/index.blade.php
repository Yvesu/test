    ...
    <div class="block">
        <ul class="nav nav-tabs nav-tabs-alt">
            <li class="{{ isActiveLabel('active',1,'active','') }}">
                <a href="{{ asset('admin/lease/index?active=1') }}">{{ trans('content.normal') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',2,'active','') }}">
                <a href="{{ asset('admin/lease/index?active=2') }}">{{ trans('content.disable') }}</a>
            </li>
        </ul>
        <div class="block-content tab-content">
            <div class="block-content tab-content">
                {{--搜索--}}
                <form action="/admin/lease/index" method="get">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tr>
                        ...
                        <th class="col-md-2">{{ trans('content.intro') }}</th>
                        <th class="hidden-xs col-md-1">{{ trans('content.cost') }}</th>
                        <th class="hidden-xs col-md-1">{{ trans('content.cost_type') }}</th>
                        <th class="hidden-xs col-md-1">{{ trans('content.ad') }}</th>
                        <th class="hidden-xs col-md-2">{{ trans('content.ad_details') }}</th>
                        <th class="hidden-xs col-md-1">{{ trans('content.lease_type') }}</th>
                    ...
                    @foreach($datas as $data)
                        <tr>
                            <td class="font-w600">{{ $data->id }}</td>
                            <td class="font-w600">{{ $data->user_id }}</td>
                            <td class="font-w600">{{ $data->hasOneIntro['intro'] }}</td>
                            <td class="hidden-xs">{{ $data->cost }}</td>
                            <td class="hidden-xs">{{ $data->cost_type }}</td>
                            <td class="hidden-xs">{{ $data->ad }}</td>
                            <td class="hidden-xs">{{ $data->ad_details }}</td>
                            <td class="hidden-xs">{{ $data->hasOneType['name'] }}</td>
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->time_add) }}</td>
                            <td class="text-center">
                                <div class="btn-group">
                                    <a class="btn btn-xs btn-default" title="编辑" href="{{ asset('/admin/lease/details?id=' . $data->id) }}"><i class="fa fa-pencil"></i></a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
