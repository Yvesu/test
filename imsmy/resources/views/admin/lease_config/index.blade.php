    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                ...
                <li><a class="link-effect" href="{{ asset('/admin/config/lease/index') }}">{{trans('common.lease_management')}}</a></li>
            </ol>
        </div>
        ...
        <div class="col-sm-8 col-xs-3">
            ...
            <div class="btn-group pull-right" id="family-action">
                <a type="button" class="btn btn-primary" href="{{ asset('/admin/config/lease/add') }}">
                    {{ trans('content.add_type') }}
                </a>
            </div>
        </div>
    </div>
                    ...
                    @foreach($datas as $k=>$data)
                        <tr class="{{ ($k%2==1)?'odd':'even' }}" {{$data -> active !=1 ? 'style=color:#CCC;' : ''}} {{ $data->pid == 0 ? 'style=background-color:#337ab7;color:#fff' : '' }}>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a class="btn btn-xs btn-default" title="编辑" href="{{ asset('/admin/config/lease/edit?id='.$data->id) }}"><i class="fa fa-pencil"></i></a>
                                </div>
                                <div class="btn-group">
                                    <a class="btn btn-xs btn-default" href="/admin/config/lease/delete?id={{ $data->id }}" title="删除"><i class="fa fa-trash"></i></a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
