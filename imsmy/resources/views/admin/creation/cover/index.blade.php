    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                <li>{{ trans('common.creation').trans('common.cover') }}</li>
                <li>
                    <a class="link-effect" href="{{ asset('/admin/creation/cover/index') }}">
                        {{ trans('common.creation').trans('common.cover').trans('common.management') }}
                    </a>
                </li>
            </ol>
        </div>
        ...
        <div class="col-sm-8 col-xs-3">
            ...
            <div class="btn-group pull-right" id="family-action">
                <a type="button" class="btn btn-primary" href="{{ asset('/admin/creation/cover/add') }}">
                    {{ trans('common.add').trans('common.cover').trans('common.file') }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="block">
        <ul class="nav nav-tabs nav-tabs-alt">
            <li class="{{ isActiveLabel('active',1,'active','') }}">
                <a href="{{ asset('/admin/creation/cover/index?active=1') }}">{{ trans('content.normal') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',3,'active','') }}">
                <a href="{{ asset('/admin/creation/cover/index?active=3') }}">{{ trans('content.recommend') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',0,'active','') }}">
                <a href="{{ asset('/admin/creation/cover/index?active=0') }}">{{ trans('content.wait') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',2,'active','') }}">
                <a href="{{ asset('/admin/creation/cover/index?active=2') }}">{{ trans('content.disable') }}</a>
            </li>
        </ul>
        <div class="block-content tab-content">
            <div class="block-content tab-content">
                {{--搜索--}}
                <form action="{{ asset('/admin/creation/cover/index') }}" method="get">
                    ...
                    @foreach($datas as $data)
                        <tr class="divStatus">
                            ...
                            <div class="row items-push js-gallery">
                                <td class="font-w600">
                                    <a href="{{ CloudStorage::downloadUrl($data->cover) }}" data-lightbox="iframe">
                            ...
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->from_time) }}</td>
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->end_time) }}</td>
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->time_add) }}</td>
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->time_update) }}</td>
                            <td class="text-center">
                                {{--<div class="btn-group">--}}
                                    {{--<a class="btn btn-xs btn-default" title="编辑" href="{{ asset('/admin/creation/cover/edit?id='.$data->id) }}"><i class="fa fa-pencil"></i></a>--}}
                                {{--</div>--}}
                                @if($data -> active == 0)
                                    <button type="button" class="btn btn-danger btn-xs" status="4" title="激活" value="{{ $data->id }}" onclick="_creationCoverSort($(this))">
                                        <i class="fa fa-check"></i>
                                    </button>
                                @endif
                                @if($data -> active == 1 && $data -> recommend != 1)
                                    <button type="button" class="btn btn-danger btn-xs" status="5" title="推荐" value="{{ $data->id }}" onclick="_creationCoverSort($(this))">
                                        <i class="fa fa-check"></i>
                                    </button>
                                @endif
                                <button type="button" class="btn btn-danger btn-xs" status="3" title="删除" value="{{ $data->id }}" onclick="_creationCoverSort($(this))">
