        ...
        <div class="block-content tab-content">
            <div class="col-md-10 col-xs-12 block">
                ...
                <form class="js-validation-topic form-horizontal"
                      ...
                      action="{{ asset('/admin/role/update') }}"
                      method="POST">
                    <input type="hidden" name="_timezone" value="Asia/Shanghai">
                    <input type="hidden" name="_token" value="{{ csrf_token()}}">
                    <input type="hidden" class="small" name="id" value="{{ $one->id }}">
                    ...
                    <div class="form-group">
                        <div class="col-md-8">
                            ...
                            <div class="form-material form-material-primary">
                                <input type="text" class="form-control" id="name" name="name" value="{{ $one->name }}" placeholder="组名称">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="col-md-8">
                            ...
                            <div class="form-material form-material-primary">
                                <input type="text" class="form-control" id="intro" name="intro" value="{{ $one->intro }}" placeholder="组描述">
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="form-material form-material-primary col-md-10 col-md-offset-2">
                                @foreach($datas as $v)
                                    <div style="{{ $v->pid == 0 ? 'margin:20px 0;padding:3px 8px;border:1px solid #ddd' : 'margin-left:20px;' }}">
                                        <input name ="menu[]" type="checkbox" {{in_array($v->id,$menuIds) ? 'checked' : ''    }} value="{{ $v->id }}" /> {{ $v->name }}
                                    </div>
                                @endforeach
                            </div>
                        </div>
