...
<div class="content">
    <div class="block">
        <div class="block-content tab-content">
            {{--搜索--}}
            <form action="/admin/role/index" method="get">
            ...
            <table class="table table-bordered table-striped js-dataTable-full">
                ...
                <tr>
                    ...
                    <th class="col-md-2">{{ trans('content.describe') }}</th>
                    <th class="hidden-xs col-md-2">{{ trans('content.role_menu') }}</th>
                ...
                @foreach($datas as $k=>$data)
                    <tr class="{{ ($k%2==1)?'odd':'even' }}">
                        ...
                        <td class="font-w600">{{ $data->intro }}</td>
                        <td class="hidden-xs">{{ $data->r_m_ids }}</td>
                        <td class="hidden-xs">{{ $data->user_name }}</td>
                        <td class="hidden-xs">{{ $data->status == 1 ? '已生效' : '已注销' }}</td>
                        <td class="hidden-xs">{{ date('Y-m-d H:i',$data->time_add) }}</td>
                        <td class="hidden-xs">{{ date('Y-m-d H:i',$data->time_update) }}</td>
                        <td class="text-center">
                            <div class="btn-group">
                                <a class="btn btn-xs btn-default" title="详情" href="{{ asset('/admin/role/details?id='.$data->id) }}"><i class="fa fa-align-justify"></i></a>
                            </div>
                            <div class="btn-group">
                                <a class="btn btn-xs btn-default" href="/admin/role/delete?id={{ $data->id }}" title="删除"><i class="fa fa-trash"></i></a>
                            </div>
                        </td>
                    </tr>
                @endforeach
