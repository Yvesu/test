    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                <li>{{ trans('common.chartlet').trans('common.folder') }}</li>
                <li>
                    <a class="link-effect" href="{{ asset('/admin/make/chartlet/folder/index') }}">
                        {{ trans('common.chartlet').trans('common.folder').trans('common.management') }}
                    </a>
                </li>
            </ol>
        </div>
        ...
        <div class="col-sm-8 col-xs-3">
            ...
            <div class="btn-group pull-right" id="family-action">
                <a type="button" class="btn btn-primary" href="{{ asset('/admin/make/chartlet/folder/add') }}">
                    {{ trans('common.add').trans('common.chartlet').trans('common.folder') }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="block">
        <ul class="nav nav-tabs nav-tabs-alt">
            <li class="{{ isActiveLabel('active',1,'active','') }}">
                <a href="{{ asset('/admin/make/chartlet/folder/index?active=1') }}">{{ trans('content.normal') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',0,'active','') }}">
                <a href="{{ asset('/admin/make/chartlet/folder/index?active=0') }}">{{ trans('content.wait') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',2,'active','') }}">
                <a href="{{ asset('/admin/make/chartlet/folder/index?active=2') }}">{{ trans('content.disable') }}</a>
            </li>
        </ul>
        <div class="block-content tab-content">
            <div class="block-content tab-content">
                {{--搜索--}}
                <form action="{{ asset('/admin/make/chartlet/folder/index') }}" method="get">
                    ...
                    @foreach($datas as $data)
                        <tr class="divStatus">
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-success btn-xs" status="1" title="上移" value="{{ $data->id }}" onclick="_chartletFolderSort($(this))">
                                    <i class="glyphicon glyphicon-arrow-up"></i>
                                </button>
                                <button type="button" class="btn btn-success btn-xs" status="2" title="下移" value="{{ $data->id }}" onclick="_chartletFolderSort($(this))">
                                    <i class="glyphicon glyphicon-arrow-down"></i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" status="3" title="删除" value="{{ $data->id }}" onclick="_chartletFolderSort($(this))">
                                    <i class="fa fa-remove"></i>
                                </button>
                                <div class="btn-group">
                                    <a class="btn btn-xs btn-default" title="编辑" href="{{ asset('/admin/make/chartlet/folder/edit?id='.$data->id) }}"><i class="fa fa-pencil"></i></a>
                                </div>
                                @if($data -> active != 1)
                                    <button type="button" class="btn btn-danger btn-xs" status="4" title="激活" value="{{ $data->id }}" onclick="_chartletFolderSort($(this))">
