    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                <li>{{ trans('common.filter').trans('common.file') }}</li>
                <li>
                    <a class="link-effect" href="{{ asset('/admin/make/filter/file/index') }}">
                        {{ trans('common.filter').trans('common.file').trans('common.management') }}
                    </a>
                </li>
            </ol>
        </div>
        ...
        <div class="col-sm-8 col-xs-3">
            ...
            <div class="btn-group pull-right" id="family-action">
                <a type="button" class="btn btn-primary" href="{{ asset('/admin/make/filter/file/add') }}">
                    {{ trans('common.add').trans('common.filter').trans('common.file') }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="block">
        <ul class="nav nav-tabs nav-tabs-alt">
            <li class="{{ isActiveLabel('active',1,'active','') }}">
                <a href="{{ asset('/admin/make/filter/file/index?active=1') }}">{{ trans('content.normal') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',3,'active','') }}">
                <a href="{{ asset('/admin/make/filter/file/index?active=3') }}">{{ trans('content.recommend') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',0,'active','') }}">
                <a href="{{ asset('/admin/make/filter/file/index?active=0') }}">{{ trans('content.wait') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',2,'active','') }}">
                <a href="{{ asset('/admin/make/filter/file/index?active=2') }}">{{ trans('content.disable') }}</a>
            </li>
        </ul>
        <div class="block-content tab-content">
            <div class="block-content tab-content">
                {{--搜索--}}
                <form action="{{ asset('/admin/make/filter/file/index') }}" method="get">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tr>
                        ...
                        <th class="col-md-2 text-center">{{ trans('content.content') }}</th>
                    ...
                    @foreach($datas as $data)
                        <tr class="divStatus">
                            ...
                            <div class="row items-push js-gallery">
                                <td class="font-w600" width="50px">
                                    <div style="max-width: 100px;max-height: 100px;">
                                        <img class="img-responsive" src="{{ CloudStorage::downloadUrl($data->cover).'?imageView2/2/h/50' }}">
                                    </div>
                                </td>
                            </div>
                            <td class="text-center">{{ $data->name }}</td>
                            <td class="text-center">{{ $data->content }}</td>
                            <td class="text-center">{{ $folder[$data->folder_id] }}</td>
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->time_add) }}</td>
                            <td class="hidden-xs">{{ date('Y-m-d H:i:s',$data->time_update) }}</td>
                            <td class="text-center">
                                {{--<div class="btn-group">--}}
                                    {{--<a class="btn btn-xs btn-default" title="编辑" href="{{ asset('/admin/make/filter/file/edit?id='.$data->id) }}"><i class="fa fa-pencil"></i></a>--}}
                                {{--</div>--}}
                                @if($data -> active == 0)
                                    <button type="button" class="btn btn-danger btn-xs" status="4" title="激活" value="{{ $data->id }}" onclick="_filterFileSort($(this))">
                                        <i class="fa fa-check"></i>
                                    </button>
                                @endif
                                @if($data -> active == 1)
                                    <button type="button" class="btn btn-success btn-xs" status="5" title="{{ 1 == $data->recommend ? '取消推荐' : '推荐' }}" value="{{ $data->id }}" onclick="_filterFileSort($(this))">
                                        <i class="glyphicon {{ 1 == $data->recommend ? 'glyphicon-thumbs-down' : 'glyphicon-thumbs-up' }}"></i>
                                    </button>
                                    @if($request['active'] == 3)
                                        <button type="button" class="btn btn-success btn-xs" status="1" title="上移" value="{{ $data->id }}" onclick="_filterFileSort($(this))">
                                            <i class="glyphicon glyphicon-arrow-up"></i>
                                        </button>
                                        <button type="button" class="btn btn-success btn-xs" status="2" title="下移" value="{{ $data->id }}" onclick="_filterFileSort($(this))">
                                            <i class="glyphicon glyphicon-arrow-down"></i>
                                        </button>
                                    @endif
                                @endif
                                <button type="button" class="btn btn-danger btn-xs" status="3" title="删除" value="{{ $data->id }}" onclick="_filterFileSort($(this))">
