    ...
    <div class="row items-push">
        ...
        <div class="col-sm-4 col-xs-9">
            <h1 class="page-heading" >
                {{trans('common.verify_management')}}
            </h1>
        </div>
    </div>
</div>
...
<div class="content">
    <div class="block">
        <ul class="nav nav-tabs nav-tabs-alt reply-status">
            <li class="{{ isActiveLabel('active',1,'active','') }}" name="reply-shield">
                <a href="{{ asset('/admin/verify/index?active=1') }}">{{ trans('content.normal') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',0,'active','') }}" name="reply-normal">
                <a href="{{ asset('/admin/verify/index?active=0') }}">{{ trans('content.wait') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',2,'active','') }}" name="reply-shield">
                <a href="{{ asset('/admin/verify/index?active=1') }}">{{ trans('common.shield') }}</a>
            </li>
        </ul>
        <div class="block-content tab-content">
            <div class="block-content tab-content">
                {{--搜索--}}
                <form action="/admin/verify/index" method="get">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    @if($datas->count())
                        ...
                        <tr>
                            ...
                            <th class="hidden-xs col-md-2 text-center">{{ trans('content.verify_type') }}</th>
                        ...
                        @foreach($datas as $data)
                            <tr id="{{ $data->id }}">
                                ...
                                <td class="hidden-xs text-center">{{ $data -> verify == 1 ? '个人认证' : '企业认证' }}</td>
                                <td class="hidden-xs text-center">{{ date('Y-m-d H:i:s',$data -> time_add) }}</td>
                                <td class="hidden-xs text-center">{{ $data -> verify_info }}</td>
                                <td class="text-center">
                                    <div class="btn-group">
                                        <a class="btn btn-xs btn-default" title="详情" href="{{ asset('/admin/verify/details?id=' . $data->id) }}"><i class="fa fa-pencil"></i></a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
