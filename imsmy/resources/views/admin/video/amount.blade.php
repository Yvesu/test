    ...
    <div class="row items-push">
        <div class="col-sm-12 hidden-xs" style="margin-bottom: 10px;">
            <ol class="breadcrumb push-10-t">
                ...
                <li>
                    <a class="link-effect" href="{{ asset('/admin/video/amount') }}">
                        {{ trans('content.amount') }}
                    </a>
                </li>
            </ol>
        </div>
    ...
    <div class="block">
        <div class="block-content tab-content">
            <div class="col-md-12 col-xs-12 block">
                {{--搜索--}}
                <form action="/admin/video/amount" method="get">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    @if($datas->count())
                        <thead>
                        <tr>
                            <th class="col-md-2 text-center">{{ trans('content.date') }}ID</th>
                            <th class="col-md-2 text-center">{{ trans('content.handle') }}</th>
                            <th class="col-md-2 text-center">{{ trans('content.amount') }}</th>
                            <th class="col-md-2 text-center">{{ trans('content.add_time') }}</th>
                            <th class="col-md-2 text-center">{{ trans('content.update_time') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($datas as $k=>$data)
                            <tr>
                                ...
                                <td class="text-center">{{ date('Y-m-d H:i:s',$data->time_update) }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    @endif
