    ...
    <div class="row items-push">
        ...
        <div class="col-sm-4 col-xs-9">
            <h1 class="page-heading" >
                {{trans('common.user_demand_management')}}
            </h1>
        </div>
    </div>
</div>
    ...
    <div class="block">
        <ul class="nav nav-tabs nav-tabs-alt">
            <li class="{{ isActiveLabel('active',1,'active','') }}">
                <a href="{{ asset('admin/demand/index?active=1') }}">{{ trans('content.normal') }}</a>
            </li>
            <li class="{{ isActiveLabel('active',2,'active','') }}">
                <a href="{{ asset('admin/demand/index?active=2') }}">{{ trans('content.disable') }}</a>
            </li>
        </ul>
        <div class="block-content tab-content">
            <div class="block-content tab-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    <thead>
                    <tr>
                        <th class="col-md-1 text-center">{{ trans('content.job') }}</th>
                        <th class="col-md-1">{{ trans('content.sponsor') }}</th>
                        <th class="col-md-3">{{ trans('content.job_condition') }}</th>
                        <th class="hidden-xs col-md-1">{{ trans('content.cost') }}</th>
                        <th class="hidden-xs col-md-1">{{ trans('content.city') }}</th>
                    ...
                    @foreach($datas as $data)
                        <tr>
                            <td class="text-center">{{ $data->hasOneJob['name'] }}</td>
                            <td class="font-w600">{{ $data->user_id }}</td>
                            <td class="font-w600">{{ $data->hasOneCondition['job_condition'] }}</td>
                            <td class="hidden-xs">{{ $data->cost }}</td>
                            <td class="hidden-xs">{{ $data->city }}</td>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a class="btn btn-xs btn-default" title="编辑" href="{{ asset('/admin/demand/details?id=' . $data->id) }}"><i class="fa fa-pencil"></i></a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
