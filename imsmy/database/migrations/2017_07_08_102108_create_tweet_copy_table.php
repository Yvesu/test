...
class CreateTweetCopyTable extends Migration {
	...
	public function up()
	{
		Schema::create('tweet_copy', function(Blueprint $table)
		{
			$table->bigInteger('id', true)->unsigned();
			$table->bigInteger('user_id')->unsigned()->index('tweet_user_id_index')->comment('用户外键');
			$table->bigInteger('retweet')->unsigned()->nullable()->index('tweet_retweet_index')->comment('转发外键');
			$table->bigInteger('original')->unsigned()->nullable()->index('tweet_original_index')->comment('要转发的原始动态的ID');
			...
			$table->integer('browse_times')->unsigned()->nullable()->default(0)->comment('观看次数,待删除');
			$table->integer('like_count')->nullable()->default(0)->comment('点赞总数,待删除');
			$table->integer('reply_count')->nullable()->default(0)->comment('评论总数,待删除');
			$table->integer('retweet_count')->unsigned()->nullable()->default(0)->comment('转发总量,待删除');
			$table->float('tweet_grade_total', 10, 0)->unsigned()->nullable()->default(0)->comment('动态评分总数,待删除');
			$table->integer('tweet_grade_times')->unsigned()->nullable()->default(0)->comment('评分次数,待删除');
