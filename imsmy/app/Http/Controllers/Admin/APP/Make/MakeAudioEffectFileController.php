...
use App\Models\Make\{MakeAudioEffectFile,MakeAudioEffectFolder};
use CloudStorage;
use Auth;
use DB;

class MakeAudioEffectFileController extends BaseSessionController
{
    ...
    public function index(Request $request)
    {
        try{
            ...
            // 判断是否为搜索目录
            if(4 == $condition)
                $search = MakeAudioEffectFolder::where('name',$search)->firstOrFail()->id;

            // 获取集合
            $datas = MakeAudioEffectFile::whereActive($active)->ofSearch($search,$condition) -> paginate($num);
            ...
            // 返回视图
            return view('admin/app/make/audioEffect/file/index',[
                'datas'     => $datas,
                'request'   => $res,
                'condition' => $cond,
                'folder'    => MakeAudioEffectFolder::pluck('name','id')
            ]);
    ...
    public function add()
    {
        return view('admin/app/make/audioEffect/file/add',[
            'folder'=>MakeAudioEffectFolder::active()->orderBy('sort')->get(['id','name'])
        ]);
    }
    ...
    public function insert(Request $request)
    {
        try{
            ...
            $file = MakeAudioEffectFile::create([
                'name'          =>  $input['name'],
                'intro'         =>  $input['intro'],
            ...
            return redirect('admin/make/audio/effect/file/index?active=0');
        } catch (ModelNotFoundException $e){
            DB::rollback();
            abort(404);
        } catch (\Exception $e){
    ...
    public function sort(Request $request)
    {
        try{
            ...
            // 获取集合
            $data = MakeAudioEffectFile::findOrFail((int)$request -> get('id'));
            ...
            switch($status){
                ...
                case 4:
                    ...
                    $folder = MakeAudioEffectFolder::findOrFail($data->folder_id);
