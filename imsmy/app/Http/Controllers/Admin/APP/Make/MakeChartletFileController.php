...
use App\Models\Make\{MakeChartletFile,MakeChartletFolder};
use CloudStorage;
use Auth;
use DB;

class MakeChartletFileController extends BaseSessionController
{
    ...
    public function index(Request $request)
    {
        try{
            ...
            // 判断是否为搜索目录
            if(4 == $condition)
                $search = MakeChartletFolder::where('name',$search)->firstOrFail()->id;

            // 判断是否为推荐
            if(3 == $active){

                // 获取集合
                $datas = MakeChartletFile::whereActive(1)
                    -> where('recommend',1)
                    -> orderBy('sort')
                ...
                // 获取集合
                $datas = MakeChartletFile::whereActive($active)
                    -> ofSearch($search,$condition)
                    -> paginate($num);
            }
            ...
            // 返回视图
            return view('admin/app/make/chartlet/file/index',[
                'datas'     => $datas,
                'request'   => $res,
                'condition' => $cond,
                'folder'    => MakeChartletFolder::pluck('name','id')
            ]);
    ...
    public function add()
    {
        return view('admin/app/make/chartlet/file/add',[
            'folder'=>MakeChartletFolder::active()->orderBy('sort')->get(['id','name'])
        ]);
    }
    ...
    public function insert(Request $request)
    {
        try{
            ...
            $result = CloudStorage::putFile(
                'chartlet/'.$file->id.'/'.str_random(10).$time.'_'.$shot_width_height.'_.'.$screen_shot->getClientOriginalExtension(),
                $screen_shot
            );
            ...
            // 贴图视频新名称
            $new_name = 'chartlet/'.$file->id.'/'.str_random(10).$time.'.'.pathinfo($file->address,PATHINFO_EXTENSION);
            $new_preview_name = 'chartlet/'.$file->id.'/'.str_random(10).$time.'.'.pathinfo(CloudStorage::downloadUrl($input['preview_key']),PATHINFO_EXTENSION);
            ...
            return redirect('admin/make/chartlet/file/index?active=0');
    ...
    public function sort(Request $request)
    {
        try{
            // 获取集合
            $data = MakeChartletFile::findOrFail((int)$request -> get('id'));
            ...
            switch($status){
                // 上移 升，sort值越小，排序越靠前
                case 1:
                    $sort_data = MakeChartletFile::where('sort','<',$data -> sort)
                        -> where('recommend',1)
                        -> orderBy('sort','DESC')
                        -> first();
                    break;
                ...
                case 4:
                    ...
                    $folder = MakeChartletFolder::findOrFail($data->folder_id);
                ...
                case 5 :
                    ...
                    // 如果为推荐，则修改sort值
                    if(1 == $data -> recommend){
                        $sort_data = MakeChartletFolder::orderBy('sort','DESC') -> first();
