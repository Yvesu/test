...
use App\Models\Make\{MakeAudioEffectFile,MakeAudioEffectFolder,MakeAudioEffectDownloadLog,MakeAudioEffectUser};
use Crypt;
use CloudStorage;
use DB;
use Auth;
 ...
 */
class MakeAudioEffectController extends BaseController
{
    ...
    public function file(Request $request)
    {
        try{
            ...
            // 获取文件夹id,默认为第一个
            $folder_id = (int)$request -> get('folder_id',MakeAudioEffectFolder::active()->orderBy('sort')->firstOrFail()->id);
            ...
            // 获取数据
            $audio = MakeAudioEffectFile::whereHas('belongsToFolder',function($q) use($folder_id) {
    ...
    public function folder()
    {
        try{
            // 获取文件夹id,默认为第一个
            $folder = MakeAudioEffectFolder::active()->orderBy('sort')->get(['id','name','count'])->all();
    ...
    public function download($id,Request $request)
    {
        try{
            ...
            // 存入用户中心
            $file_user = MakeAudioEffectUser::where('user_id',$id)->where('file_id',$file_id)->first();

            // 开启事务
            DB::beginTransaction();

            if(!$file_user)
                MakeAudioEffectUser::create([
                    'file_id'       => $file_id,
                    'user_id'       => $id,
                    'time_add'      => $time,
                    'time_update'   => $time,
            ...
            // 存入日志文件
            MakeAudioEffectDownloadLog::create([
                'file_id'   => $file_id,
                'user_id'   => $id,
                'time_add'  => $time,
            ]);
